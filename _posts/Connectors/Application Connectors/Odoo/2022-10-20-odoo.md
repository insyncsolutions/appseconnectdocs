...
keywords: "Odoo Configuration, Odoo Application in OP Agent, Odoo Application in Cloud Agent"
toc: true
tag: developers
category: "Connectors"
menus: 
...
to fulfill your request. From the `API` structure, we are using `domain` keyword as the key and mention the `id`s of the users in the value field. 
```
{
    "success": true,
