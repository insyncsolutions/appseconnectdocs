...
1. [Create a processflow](/getting%20started/create-your-first-processflow/) with SAP ECC as source or destination application, and [deploy](/processflow/deploying-and-executing-processflow/) the processflow in On-Premise agent.  
...
`SAP IDOC` is an object that transports business transaction data in the form of an electronic message from one system to another. IDOC is an acronym for Intermediate Document. 
...
1. The administration section (Control Record) contains information such as the IDOC type, message type, current status, sender, and receiver. 

2. The data is stored in the application data (Data Record). 
...
- Click on ‘Create’ button. Provide the inputs RFC Destination, Connection Type, Description, Target Host, Service No and Path Prefix(if you do not have the Agent set up as of now, please provide a dummy value in Target Host and Path Prefix fields 
